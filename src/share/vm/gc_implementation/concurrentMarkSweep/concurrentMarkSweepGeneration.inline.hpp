...
inline bool CMSCollector::should_abort_preclean() const {
  ...
  /*
  我们在 一次可中断预清理和 要么清除已经完成要么foregc想要接管回收 的中间
  */
...
inline size_t CMSCollector::get_eden_used() const {//得到eden区使用的
  return _young_gen->as_DefNewGeneration()->eden()->used();
}

inline size_t CMSCollector::get_eden_capacity() const {//得到eden区的容量

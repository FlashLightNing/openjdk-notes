...
bool ConcurrentMarkSweepThread::_should_terminate = false;//只有在线程的stop方法中，该变量才会true
...
ConcurrentMarkSweepThread::ConcurrentMarkSweepThread(CMSCollector* collector)
  ...
  if (os::create_thread(this, os::cgc_thread)) {
    ...
    if (UseCriticalCMSThreadPriority) {//UseCriticalCMSThreadPriority默认false
      ...
      native_prio = os::java_to_os_priority[NearMaxPriority];//NearMaxPriority=9
    }
...
//_should_terminate只有在stop方法中才会为true，所以只要没有调用stop方法，这个就是while循环
...
void ConcurrentMarkSweepThread::wait_on_cms_lock_for_scavenge(long t_millis) {
  ...
  double start_time_secs = os::elapsedTime();//虚拟机启动后到现在经过的时间
  //计算出结束时间：当前时间（虚拟机启动后到现在经过的时间）+等待时间(t_millis)。
  ...
  while(!_should_terminate) {//while循环
    ...
    //等待到下一次事件或者超过剩余的等待时间
    {
      ...
      if (_should_terminate || _collector->_full_gc_requested) {//如果发起了一次full gc则结束自旋
        return;
      }
      set_CMS_flag(CMS_cms_wants_token);   // to provoke notifies
...
void ConcurrentMarkSweepThread::sleepBeforeNextCycle() {
  while (!_should_terminate) {
    if (CMSIncrementalMode) {//CMS增量模式，默认false
    //增量模式经常暂停CMS过程，以便对应用程序线程作出完全的让步。因此，收集器将花更长的时间完成整个收集周期
      icms_wait();
      /*
      CMSWaitDuration 的意思是CMS线程等待young gc的时间，默认是2000毫秒。
      之所以这样，是因为存在这样的情况：cms触发的时候,young中的eden区对象较多，进行初始标记比较费时间，
      所以等待2秒，希望这中间能进行一次young gc回收一些eden中死亡的对象
      */

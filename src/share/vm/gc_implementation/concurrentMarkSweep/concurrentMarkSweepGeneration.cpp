...
// durations.
//当CMS线程和VM线程都在回收过程中不希望被对方线程干扰，它就需要获取CMS token.
//我们将CGC_LOCK的用途分为2个等级。低等级的锁就是通常的CGC_LOCK 监视器。我们引入了高等级的token
...
//如果一个线程已经获取了token，那另外一个线程就要等待。当CMS线程和VM线程都想要获取token时，VM线程会优先获取。
//这是为了保证在并发阶段中，CMS的线程不会因为长时间的占有token而导致阻塞VM线程过长的时间
...
//由_foregroundGCShouldWait 和_foregroundGCIsActive控制的
...
class CMSParGCThreadState: public CHeapObj<mtGC> {
 public:
  CFLS_LAB lab;//和TLAB相似
  PromotionInfo promo;//担保信息
...
void ConcurrentMarkSweepGeneration::init_initiating_occupancy(intx io, uintx tr) {
  ...
  } else {//所以会走下面的逻辑，MinHeapFreeRatio默认40
...
double CMSStats::time_until_cms_start() const {
  ...
  double deadline = time_until_cms_gen_full();//预计cms代要满的时间
  ...
  //如果一次concurrent mode faliure刚刚发生，我们想更加谨慎的然后将我们期望的time_until_cms_gen_full时间一分为二。
  if (work > deadline) {
  ...
  _cms_start_registered(false)
{
  ...
  */
  ...
  /*并行任务队列。这些队列由CMS的并发和STW阶段共享，但是不和parallelscavenge共享
  */
  {
    uint i;
...
void ConcurrentMarkSweepGeneration::reset_after_compaction() {
  ...
  //
  /*
  Clear the promotion information.  
  These pointers can be adjusted along with all the other pointers into the heap 
  but compaction is expected to be a rare event with  a heap using cms so don't do it 
  without seeing the need.
  清除晋升的信息。这些指针可以随所有其他指针一起调整到堆中
  但是，使用cms的情况下，压缩是一种罕见的事件。所以如果不是必要，就不要使用该方法
  */
  if (CollectedHeap::use_parallel_gc_threads()) {
    for (uint i = 0; i < ParallelGCThreads; i++) {
...
bool CMSCollector::shouldConcurrentCollect() {
  if (_full_gc_requested) {//只有在request_full_gc()方法内，该变量才为true
    if (Verbose && PrintGCDetails) {
  ...
  */
  if (!UseCMSInitiatingOccupancyOnly) {
    if (stats().valid()) {
      ...
      if (_cmsGen->occupancy() >= _bootstrap_occupancy) {//默认50%
        if (Verbose && PrintGCDetails) {
          gclog_or_tty->print_cr(
  ...
  */
  ...
  /* We start a collection if we believe an incremental collection may fail;
   ...
   late anyway.
  如果我们认为一个增量式的回收可能失败，我们就会开始一次回收。这在实践中不太可能生效，因为这时候已经太迟了。
  */
  GenCollectedHeap* gch = GenCollectedHeap::heap();
...
/* We should be conservative in starting a collection cycle.  To
 ...
 collections.
如果下面的任何一个条件满足，我们就会开始一次回收
 We want to start a new collection cycle if any of the following
 conditions hold:
 我们的占用率超过了对这个堆配置的占用率。
 或者我们最近需要增大堆的空间，同时，在扩大之后，还没有对这个堆进行过回收
 或者底部的空间认为这是一个很好的开始并发回收的机会。（这是基于以下条件的，例如：
 该空间使用线性分配，而且线性分配快失败了。或者在该堆中有过多的内存碎片或者。。
 ）
 ...
 [.(currently done by CMSCollector::shouldConcurrentCollect() only for
   ...
   我们也许接近了这样的情况：分配内存的请求可能会失败，因为根据对分配率的估计，我们快用完了空闲空间】
...
/*
请求进行一次fullgc，只有在这中，_full_gc_requested才会赋值
*/
  ...
  while (_collectorState != Idling) {
    ...
    switch (_collectorState) {
      ...
      case Marking:
        ...
        if (markFromRoots(true)) { // we were successful backgroundgc
...
void CMSCollector::register_gc_start(GCCause::Cause cause) {
  ...
  _gc_timer_cm->register_gc_start();//开始计时
...
void CMSCollector::collect_in_foreground(bool clear_all_soft_refs, GCCause::Cause cause) {
  ...
  while (_collectorState != Idling) {//注意是while循环，只要不等于idle，就会一直处理
    if (TraceCMSState) {
      gclog_or_tty->print_cr("Thread " INTPTR_FORMAT " in CMS state %d",
        Thread::current(), _collectorState);
    }
    switch (_collectorState) {
      case InitialMarking:
        ...
        checkpointRootsInitial(false);//在该方法里面会将_collectorState设置为marking
      ...
      case Marking:
        ...
        {
          bool res = markFromRoots(false);//foreground
...
// below.
/*
判断是否做类型卸载。决定我们是否想让类型卸载作为并发GC周期后的一个阶段，如果在以下的情况下，我们会收集垃圾然后卸载类：

1.一个显式GC（system gc）被调用，且设置了ExplicitGCInvokesConcurrentAndUnloadsClasses参数
或者2：类型卸载在命令行中支持，且老年代快满了。
2.a.类型卸载在命令行中配置打开了，且b.老年代快满了。
注意：如果在这个方法调用之间堆的状态没有变化，那么它需要保证幂等性。
另外，如果没有垃圾回收，在2次调用之间，它的结果应该是单调递增的（例如从0到1，但是不会从1到0）
在下面的实现中，它必须依赖这样的属性：concurrent_cycles_since_last_unload()不会降低除非一个回收周期发生而且
 _cmsGen->is_too_full() 是单调的。 看一下下面的check_monotonicity()方法。
*/
void CMSCollector::update_should_unload_classes() {
  ...
  } else if (CMSClassUnloadingEnabled) { // 上面的条件2
    // Disjuncts 2.b.(i,ii,iii) above
    /*自从上次卸载到现在发生的次数>=设置的CMS类型卸载的周期（默认0），
    或者当前堆的使用量大于参数CMSIsTooFullPercentage设置的比例
    */
...
bool ConcurrentMarkSweepGeneration::is_too_full() const {
  bool res = should_concurrent_collect();//是否需要并发收集？还需要判断？
  //如果确实需要并发收集，且堆的使用量大于配置的占用量
...
void CMSCollector::setup_cms_unloading_and_verification_state() {
  const  bool should_verify =   VerifyBeforeGC || VerifyAfterGC || VerifyDuringGC
                             || VerifyBeforeExit;
  const  int  rso           =   SharedHeap::SO_AllCodeCache;//S0区域的代码缓存

  // We set the proper root for this CMS cycle here.
  if (should_unload_classes()) {   // Should unload classes this cycle
    remove_root_scanning_option(rso);  // Shrink the root set appropriately 适当地缩小根集合
    set_verifying(should_verify);    // Set verification state for this cycle 验证状态
...
/*
Parallel initial mark task  并发标记
CMSParInitialMarkTask 和CMSParRemarkTask 共同的部分
*/
...
/* Checkpoint the roots into this generation from outside this generation. 
[Note this initial checkpoint need only
...
/*
从根开始标记，参数：是否异步.
在back gc时，asynch=TRUE
在fore gc时，asynch=false
*/
bool CMSCollector::markFromRoots(bool asynch) {
  ...
  if (asynch) {
    ...
    if (res) {
      _collectorState = Precleaning;//下一步是预清理，表示是backgroundgc
  ...
  } else {//如果是fore gc，mark完后就进入最终标记阶段
    assert(SafepointSynchronize::is_at_safepoint(),
    ...
    // already have locks
    res = markFromRootsWork(asynch);//在这里asynch=false;
    _collectorState = FinalMarking;//下一步是最终标记,可能是foreground
    if (UseAdaptiveSizePolicy) {
      GenCollectedHeap* gch = GenCollectedHeap::heap();
...
bool CMSCollector::markFromRootsWork(bool asynch) {
  ...
  /*
  在位图中遍历标记位，使用下面的算法做一个全扫描以及从这些根开始标记：
  如果oop在指针扫描的右边，标记相应的位（我们后面会处理他）
  否则（oop在当前扫描指针的左边时），将oop放在标记的堆栈上，然后清空堆栈
  ...
  */
  ...
  //   push oop on marking stack drain the marking stack

/*
需要注意的是，当我们做标记阶段时，我们需要持有位图的锁。回想一下：内存分配和晋升也同时在标记位图[所谓的分配策略]
因为同时在同一个字节中进行标记的话，位图标记的实现的鲁棒性不够强。
我们需要保证在诸如此类并发中，比如更新，不会相互影响
*/ 
  ...
  // Because the implementation of bit map marking is not robust wrt simultaneous marking of bits in the same word,
  ...
  bool result = false;
  /*
  如果配置的 CMSConcurrentMTEnabled参数和ConcGCThreads >0，
  就backgc:do_marking_mt(TRUE);foregc:do_marking_mt(false)

  否则就是:
  CMSConcurrentMTEnabled 默认true
  ConcGCThreads 默认0
  如果还标志未设置，JVM会根据并行收集器中的-XX：ParallelGCThreads参数的值来计算出默认的并行CMS线程数。
  该公式是ConcGCThreads = (ParallelGCThreads + 3)/4。
cms最开始相当于构造器的地方，有如下一段代码
  if (FLAG_IS_DEFAULT(ConcGCThreads)) {
      // just for now
      FLAG_SET_DEFAULT(ConcGCThreads, (ParallelGCThreads + 3)/4);
    }
  */
  if (CMSConcurrentMTEnabled && ConcGCThreads > 0) {
    result = do_marking_mt(asynch);//multiple thread 多线程
  } else {
    result = do_marking_st(asynch);//single thread 单线程
  }
  return result;
}
...
class CMSConcMarkingTerminator: public ParallelTaskTerminator {
 ...
 public:
  ...
  /*
  n_threads是需要被中止的线程数
  queue_set是线程的工作队列的集合
  collector是与该被中止的任务关联的回收器
  yiled 表示我们是否需要
  */
...
class CMSConcMarkingTask: public YieldingFlexibleGangTask {
  ...
  bool          _asynch;
  bool          _result;
  CompactibleFreeListSpace*  _cms_space;//可压缩的空闲列表区域
  char          _pad_front[64];   // padding to ...
  HeapWord*     _global_finger;   // ... avoid sharing cache line避免共享缓存行
  char          _pad_back[64];
...
/*并发标记算法描述
 Until all tasks exhausted (both spaces):  直到所有任务完成（2个空间）
 -- claim next available chunk 认领下一个回收的区域
 -- bump global finger via CAS 通过CAS和指针碰撞
 -- find first object that starts in this chunk
    and start scanning bitmap from that position
  找到该区域中的第一个开始的对象，然后从这个位置开始扫描位图
 -- scan marked objects for oops扫描标记的对象
 -- CAS-mark target, and if successful:用CAS来标记对象，如果成功了：
 ...
 -- Deal with possible overflow issues:处理可能溢出的问题
...
在工作偷取阶段的灰色对象扫描
假设是：在这被偷取的对象中的任何被扫描的对象必须依据被初始化（否则他们不会被发布出来）。所以我们不需要检查
...
bool CMSCollector::do_marking_mt(bool asynch) {
  ...
  // below? In particular, may be we need to subclass the SequantialSubTasksDone class?? XXX
  /*
  既然我们实际得到的线程的数量也许会和我们设置的数量不一样。我们需要在接下来做一些不同的事情吗？
  */
...
void CMSCollector::preclean() {
  ...
  verify_overflow_empty();
  _abort_preclean = false; 
  if (CMSPrecleaningEnabled) {//CMSPrecleaningEnabled默认true  whether concurrent precleaning enabled
    if (!CMSEdenChunksRecordAlways) {
      //always record eden chunks used for the parallel initial mark or remark of eden ,jdk8默认true,《=7 false
      _eden_chunk_index = 0;
    }
...
void CMSCollector::abortable_preclean() {
  ...
  /* If Eden's current occupancy is below this threshold,
   immediately schedule the remark; 
   else preclean past the next scavenge in an effort to schedule the pause as described avove. 
   By choosing
  ...
  */
  if (get_eden_used() > CMSScheduleRemarkEdenSizeThreshold) {
    ...
    while (!(should_abort_preclean() ||
      ...
      if (pa.wallclock_millis() > CMSMaxAbortablePrecleanTime) {//预清理阶段最大耗时 默认值5000毫秒
        if (PrintGCDetails) {
...
size_t CMSCollector::preclean_work(bool clean_refs, bool clean_survivor) {
  ...
  /*
  预清理并不是并发的，但是引用处理器也许是设置的并发。在这里让他先不可用
  */
  ReferenceProcessor* rp = ref_processor();
...
// one of these methods, please check the other method too.
/*
preclean_mod_union_table 和preclean_card_table方法大部分都相同。
如果你需要修改其中的一个方法，请核对一下另外的一个方法。
...
void CMSCollector::checkpointRootsFinal(bool asynch,
  ...
  if (asynch) {
    /*
    CMSScavengeBeforeRemark 重新标记之前对年轻代做一次minor GC
    */
...
void CMSCollector::checkpointRootsFinalWork(bool asynch,
  ...
  if (!init_mark_was_synchronous) {
...

    {
      ...
      /*
      mod union table的作用：
      因为在markfromroots阶段中,标记线程并发进行标记。一些可达对象没有被扫描到是有可能的。
      例如：一个指向A的唯一的引用在B被扫描完后替换了B的位置，除非B被再次扫描，A才会被收集。
      这样情况的更新操作可以通过mod union table被检测到。mod union table是自从第一次GC检查后，所有脏的卡表的集合。

      CMSParallelRemarkEnabled 默认true 
      */
      if (CMSParallelRemarkEnabled && CollectedHeap::use_parallel_gc_threads()) {
        GCTraceTime t("Rescan (parallel) ", PrintGCDetails, false, _gc_timer_cm, _gc_tracer_cm->gc_id());
        do_remark_parallel();//并行的的重新标记，因为重新标记阶段是要STW的
      } else {
...
// Parallel version of remark 并行版本的重新标记
...
// Non-parallel version of remark非并行的做重新标记

...
class frame VALUE_OBJ_CLASS_SPEC {
 private:
  // Instance variables:
  intptr_t* _sp; // stack pointer (from Thread::last_Java_sp)指向栈顶元素
  address   _pc; // program counter (the next instruction after the call)指向下一条要执行的指令地址

  CodeBlob* _cb; // CodeBlob that "owns" pc指向持有相应的指令机器码的CodeBlob
  enum deopt_state {
    not_deoptimized,
    is_deoptimized,
    unknown
